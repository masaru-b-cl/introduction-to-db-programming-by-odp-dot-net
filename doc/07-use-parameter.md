...
この例ではSELECT句でパラメーターを利用していましたが、当然INSERT/UPDATE/DELETE等のDMLでもパラメーターは利用できます。なお、2つ以上パラメーターを使用する場合、SQL文中の順番とパラメーターを追加する順番を揃えないといけないことには注意してください。

## パラメーターの他の利点

SQLインジェクションの脆弱性への対策の他にも、パラメーターを利用する利点があります。

SQLを実行する際、DBではそのSQLを解析して実行しています。ただ、パフォーマンス上の都合から、同じSQLが実行された際はこの解析をスキップして実行するようになっていることが多いです。

つまり、文字列連結でSQLを組み立てた場合、毎回解析は行われることになります。これをパラメーターを利用するものに置き換えると、DBで解析されるSQLはパラメーターが埋め込まれた同一のものになるため、2回目移行は解析が行われません。したがって、パラメーターを使うことは、パフォーマンス上でも有利になるということです。


以上のことから、なにか動的に値を変更するケースでは、必ずパラメーターを利用するようにしましょう。仮に条件自体の数が増減するにしても、それに合わせてパラメーターの数も増減させることで、対応は可能です。


これで安全なSQL文実行を行うことが出来るようになりました。次の章では、ストアド・プロシージャの呼び出し方を学んでいきましょう。
