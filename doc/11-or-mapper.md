...
11章にわたって.NET データ プロバイダーを用いたDBアクセスについて説明してきました。本文書の内容で、まず大体のDBアクセス処理は行えるはずです。

しかし、実際のアプリケーション開発においては、どうしても各DB製品、データ プロバイダーの機能を使わざるをえない場合もあります。そういった時は本書の内容を応用し、各製品のマニュアル等を参考に対処してもらえればと思います。
